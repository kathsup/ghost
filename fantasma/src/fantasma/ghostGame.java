...
public class ghostGame {
    ...
    public static String login() {
        ...
        // Buscar al jugador en el arreglo
    ...
    public static void crearPlayer() {
        ...
        if (playerCount >= 100) {
            System.out.println("Error: No se pueden crear más jugadores.");
            return;
        }
        ...
        playerCount++;
        System.out.println("Player creado exitosamente. Cargando menú principal...");
        menuPrincipal();
    }
  ...
  public static String jugador2() {
    ...
    public static void mostrarTablero() {
        //los numeros del tablero
         System.out.print("  "); // Espacio inicial para la primera fila de números
    for (int i = 0; i <= 5; i++) {
        System.out.print(i + " ");
    }
    System.out.println();
    ...
    for (int i = 0; i < 6; i++) {
        ...
        // Imprimir las celdas del tablero
        for (int j = 0; j < 6; j++) {
            System.out.print(tableroInvisible[i][j] + " ");
        }
        System.out.println(); // Salto de línea para la siguiente fila
    }
    ...
    public static void colocarFantasmas1M(int cantidadFantasmas) {
        ...
        for (int i = 0; i < cantidadFantasmas; i++) {
            ...
            System.out.println(jugador1.getUsername() + " Ingresa la posicion para el fantasma " + (i + 1) + " (Fila/columna)");
    ...
    Random randomf = new Random(2); 
    Random randomc = new Random(6);
        for (int i = 0; i < cantidadFantasmas; i++) {
         String tipoFantasma = (i % 2 == 0) ? "b" : "m";
         int fila=randomf.nextInt(2);
         int columna = randomc.nextInt(6); 
    ...
    public static void colocarFantasmas2M(int cantidadFantasmas) {
        ...
        for (int i = 0; i < cantidadFantasmas; i++) {
            ...
            System.out.println(jugador2.getUsername() + " jugador 2 Ingrese posicion para fantasma " + (i + 1) + " (Fila/columna)");
            int fila = entrada.nextInt();
            int columna = entrada.nextInt();
    ...
    public static boolean ganar() {
        ...
        System.out.println("Verificando si hay un ganador...ganar");
        System.out.println("fantasmasB: " + fantasmasB + " fantasmasb: " + fantasmasb);
        System.out.println("fantasmasM: " + fantasmasb + " fantasmasm: " + fantasmasm);

        if (jugador1Retiro) {
        System.out.println(jugador2.getUsername() + " ha ganado porque " + jugador1.getUsername() + " se retiró.");
        jugador2.sumarPuntos(3); // Sumar puntos al ganador
        jugador2.agregarJuegoAlHistorial(jugador2.getUsername() + " ganó porque " + jugador1.getUsername() + " se retiró.");
       // actualizarRanking();
        juegoterminado = true;
    } else if (jugador2Retiro) {
        System.out.println(jugador1.getUsername() + " ha ganado porque " + jugador2.getUsername() + " se retiró.");
        jugador1.sumarPuntos(3); // Sumar puntos al ganador
        jugador1.agregarJuegoAlHistorial(jugador1.getUsername() + " ganó porque " + jugador2.getUsername() + " se retiró.");
        //actualizarRanking();
        juegoterminado = true;
    }
        if (fantasmasb == 0) {
    ...
    if (retiro.equals("s")) {
        ...
        System.out.println("El juego continuará.");
        return false;  
    } else {
        System.out.println("Opción inválida, el juego continuará.");
        return false;  
    }
}
    ...
    while (!movimientoValido) {
        ...
        if (MovimientoValido(fila, columna, nuevaFila, nuevaColumna, 1)) {  
            ...
            if (tableroVisible[nuevaFila][nuevaColumna].equals("F")) {
    ...
    if (jugador == 1 && tableroVisible[fila][columna].equals("F")) { // Jugador 1 captura fantasmas del Jugador 2
        if (tableroInvisible[fila][columna].equals("B")) {
            System.out.println("El Jugador 1 capturó un fantasma bueno del Jugador 2");
            fantasmasB--;  // Reducir el número de fantasmas buenos del Jugador 2
        } else if (tableroInvisible[fila][columna].equals("M")) {
            System.out.println("El Jugador 1 capturó un fantasma malo del Jugador 2");
            fantasmasM--;  // Reducir el número de fantasmas malos del Jugador 2
        }
    } else if (jugador == 2 && tableroVisible[fila][columna].equals("f")) { // Jugador 2 captura fantasmas del Jugador 1
        if (tableroInvisible[fila][columna].equals("b")) {
            System.out.println("El Jugador 2 capturó un fantasma bueno del Jugador 1");
            fantasmasb--;  // Reducir el número de fantasmas buenos del Jugador 1
        } else if (tableroInvisible[fila][columna].equals("m")) {
            System.out.println("El Jugador 2 capturó un fantasma malo del Jugador 1");
            fantasmasm--;  // Reducir el número de fantasmas malos del Jugador 1
        }
    }

    // Limpiar la casilla después de la captura
    tableroVisible[fila][columna] = "-";
    tableroInvisible[fila][columna] = "-";

    // Mostrar el estado de los fantasmas restantes
    System.out.println("Fantasmas restantes:   capturar  " + fantasmasB + " fantasmas buenos (Jugador 2), " + fantasmasM + " fantasmas malos (Jugador 2)");
    System.out.println("Fantasmas restantes: " + fantasmasb + " fantasmas buenos (Jugador 1), " + fantasmasm + " fantasmas malos (Jugador 1)");
}
    ...
    for (int i = 0; i < 4; i++) {
        int fila = random1.nextInt(2);  
        int columna = random2.nextInt(6);  
        
       
        if ((fila == 0 && columna > 0 && columna < 5) || (fila == 1 && columna >= 0 && columna <= 5) && tableroInvisible[fila][columna].equals("-")) {
    ...
    public static void colocarFantasmasTrampa2() {
     ...
     Random random2 = new Random(6);
    for (int i = 0; i < 4; i++) {
        int fila = random1.nextInt(2)+4;  
        int columna = random2.nextInt(6); 
        
        
        if ((fila == 5 && columna > 0 && columna < 5) || (fila == 4 && columna >= 0 && columna <= 5) && tableroInvisible[fila][columna] == "-") {
            tableroInvisible[fila][columna] = "T";  
            tableroVisible[fila][columna] = "F";  
        } else {
            i--; 
    ...
    public static void ultimos10juegos(){
     ...
     if (jugador1 != null) {
        System.out.println("------ Últimos 10 Juegos ------");
